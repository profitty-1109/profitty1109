  ...
  useEffect(() => {
    const fetchReservations = async () => {
      try {
        ...
        // 응답 상태 코드 확인 및 로깅
        console.log("예약 API 응답 상태:", response.status)

        if (!response.ok) {
          const errorData = await response.json().catch(() => ({}))
          console.error("예약 API 오류 응답:", errorData)
          ...
          if (response.status === 401 && errorData.requireAuth) {
            ...
            // 3초 후 로그인 페이지로 리다이렉트
            setTimeout(() => {
              window.location.href = "/login?redirect=/resident/facilities/my-reservations"
            }, 3000)
      ...
      <div className="flex-1">
        ...
        <main className="p-4 md:p-6 space-y-6">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold">내 시설 예약</h1>
            <Button asChild>
          ...
          <Tabs defaultValue="upcoming">
            ...
            <TabsContent value="upcoming" className="space-y-4">
              {isLoading ? (
                      ...
                      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div className="flex items-start gap-4">
                          ...
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              <h3 className="font-medium">{reservation.facilityName}</h3>
                        ...
                        <div className="flex items-center gap-2 self-end md:self-auto">
                          <Button variant="outline" size="sm" asChild>
                            <Link href={`/resident/facilities/${reservation.facilityId}`}>상세 정보</Link>
                          </Button>
                          <Dialog>
                ...
                <div className="text-center py-10">
                  ...
                  <h3 className="font-medium text-lg mb-2">예정된 예약이 없습니다</h3>
                  <p className="text-muted-foreground mb-4">새로운 시설을 예약해보세요!</p>
                  <Button asChild>
                    <Link href="/resident/facilities">시설 예약하기</Link>
                  </Button>
                </div>
            ...
            <TabsContent value="past" className="space-y-4">
              {isLoading ? (
                ...
                <div className="text-center py-10">
                  <Calendar className="h-10 w-10 mx-auto text-muted-foreground mb-2" />
                  <h3 className="font-medium text-lg mb-2">지난 예약이 없습니다</h3>
                  <p className="text-muted-foreground mb-4">새로운 시설을 예약해보세요!</p>
                  <Button asChild>
                    <Link href="/resident/facilities">시설 예약하기</Link>
                  </Button>
