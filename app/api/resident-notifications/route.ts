...
export async function GET(request: NextRequest) {
  try {
    ...
    // 사용자 알림 설정 데이터 가져오기 (실제 구현에서는 데이터베이스에서 가져옴)
    ...
    console.error("Error fetching resident notification settings:", error)
...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.log(`Updating notification settings for user ${userId}:`, data)

    return NextResponse.json({ success: true, message: "알림 설정이 성공적으로 업데이트되었습니다." }, { status: 200 })
  } catch (error) {
    console.error("Error updating resident notification settings:", error)
